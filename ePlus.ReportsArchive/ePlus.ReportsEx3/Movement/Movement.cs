...
namespace Movement
{
	public class Movement : AbstractDocumentReport, IExternalDocumentPrintForm
	{
		...
		{
			Stream s = this.GetType().Assembly.GetManifestResourceStream("Movement.Movement.sql");
		...
		void ExtractReport()
		{
			...
			Stream s = this.GetType().Assembly.GetManifestResourceStream("Movement.Movement.rdlc");
		...
		public override IReportForm GetReportForm(DataRowItem dataRowItem)
		{
			...
			{
				summary += Utils.GetDecimal(Row, "PRICE_SUMMA_VAT");				
				i++;
			}
		...
		{
			get { return "Перемещение товара: Перемещение"; }
		}
	}
}
