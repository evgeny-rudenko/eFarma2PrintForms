...
namespace FCSInvoiceOutInvoice
{
	public class InvoiceOutInvoiceEx : AbstractDocumentReport, IExternalDocumentPrintForm
	{
		...
		{
            Stream s = this.GetType().Assembly.GetManifestResourceStream("FCSInvoiceOutInvoice.InvoiceOutInvoice.sql");
		...
		void ExtractReport()
		{
            ...
            Stream s = this.GetType().Assembly.GetManifestResourceStream("FCSInvoiceOutInvoice.InvoiceOutInvoice.rdlc");
            ...
            if (!Directory.Exists(cachePath))
                Directory.CreateDirectory(cachePath);
            s = this.GetType().Assembly.GetManifestResourceStream("FCSInvoiceOutInvoice.InvoiceOutInvoiceAfter240112.rdlc");
            using (StreamReader sr = new StreamReader(s))
            {
                using (StreamWriter sw = new StreamWriter(Path.Combine(cachePath, "InvoiceOutInvoiceAfter240112.rdlc")))
                {
                    sw.Write(sr.ReadToEnd());
                }
            }
		...
		public override IReportForm GetReportForm(DataRowItem dataRowItem)
		{
            ...
            rep.ReportPath = Path.Combine(Path.Combine(folderPath, CACHE_FOLDER), "InvoiceOutInvoiceAfter240112.rdlc");
