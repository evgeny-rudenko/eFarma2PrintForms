...
namespace InvoiceOutInvoiceEx
{
	public class BillInvoiceEx : AbstractDocumentReport, IExternalDocumentPrintForm
	{
		...
		{
			Stream s = this.GetType().Assembly.GetManifestResourceStream("BillInvoiceEx.BillInvoice.sql");
		...
		void ExtractReport()
		{
			...
			Stream s = this.GetType().Assembly.GetManifestResourceStream("BillInvoiceEx.BillInvoice.rdlc");
			using (StreamReader sr = new StreamReader(s))
			{
				using (StreamWriter sw = new StreamWriter(Path.Combine(cachePath, "BillInvoice.rdlc")))
				{
            ...
            if (!Directory.Exists(cachePath))
                Directory.CreateDirectory(cachePath);
            s = this.GetType().Assembly.GetManifestResourceStream("BillInvoiceEx.BillInvoiceAfter240112.rdlc");
            using (StreamReader sr = new StreamReader(s))
            {
                using (StreamWriter sw = new StreamWriter(Path.Combine(cachePath, "BillInvoiceAfter240112.rdlc")))
                {
                    sw.Write(sr.ReadToEnd());
                }
            }
		...
		public override IReportForm GetReportForm(DataRowItem dataRowItem)
		{
			...
			rep.ReportPath = Path.Combine(Path.Combine(folderPath, CACHE_FOLDER), "BillInvoice.rdlc");
            else
            rep.ReportPath = Path.Combine(Path.Combine(folderPath, CACHE_FOLDER), "BillInvoiceAfter240112.rdlc");

			rep.LoadData("REPEX_BILL_INVOICE", doc.InnerXml);
        ...
        private DateTime GetDateDocument(Guid guid)
        {
            ...
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                SqlCommand command = new SqlCommand(string.Format("SELECT DOC_DATE FROM BILL WHERE ID_BILL_GLOBAL = '{0}'", guid), con);
