...
namespace FCSBillInvoiceNV
{
    public class FCSBillInvoiceNV : AbstractDocumentReport, IExternalDocumentPrintForm
	{
		...
		{
            Stream s = this.GetType().Assembly.GetManifestResourceStream("FCSBillInvoiceNV.BillInvoiceNV.sql");
		...
		void ExtractReport()
		{
            ...
            Stream s = this.GetType().Assembly.GetManifestResourceStream("FCSBillInvoiceNV.BillInvoiceNV.rdlc");
            ...
            if (!Directory.Exists(cachePath))
                Directory.CreateDirectory(cachePath);
            s = this.GetType().Assembly.GetManifestResourceStream("FCSBillInvoiceNV.BillInvoiceNVAfter240112.rdlc");
            using (StreamReader sr = new StreamReader(s))
            {
                using (StreamWriter sw = new StreamWriter(Path.Combine(cachePath, "BillInvoiceNVAfter240112.rdlc")))
                {
                    sw.Write(sr.ReadToEnd());
                }
            }
		...
		public override IReportForm GetReportForm(DataRowItem dataRowItem)
		{
            ...
            rep.ReportPath = Path.Combine(Path.Combine(folderPath, CACHE_FOLDER), "BillInvoiceNVAfter240112.rdlc");
