...
namespace MovementKirov
{
	public class MovementEkaterinburgEx : AbstractDocumentReport, IExternalDocumentPrintForm
	{
		...
		{
			Stream s = this.GetType().Assembly.GetManifestResourceStream("MovementKirov.MovementKirov.sql");
		...
		void ExtractReport()
		{
			...
			Stream s = this.GetType().Assembly.GetManifestResourceStream("MovementKirov.MovementKirov.rdlc");
			using (StreamReader sr = new StreamReader(s))
			{
				using (StreamWriter sw = new StreamWriter(Path.Combine(cachePath, "MovementKirov.rdlc")))
				{
		...
		public override IReportForm GetReportForm(DataRowItem dataRowItem)
		{
			...
			rep.ReportPath = Path.Combine(Path.Combine(folderPath, CACHE_FOLDER), "MovementKirov.rdlc");

			rep.LoadData("REPEX_MOVEMENT_KIROV", doc.InnerXml);
		...
		{
			get { return "Перемещение товара: Перемещение Киров"; }
		}
	}
