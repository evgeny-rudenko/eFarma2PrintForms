...
IF (OBJECT_ID('DBO.FN_CERT_LIST_4_REPEX_MOVEMENT_KIROV') IS NULL)	EXEC ('CREATE FUNCTION DBO.FN_CERT_LIST_4_REPEX_MOVEMENT_KIROV() RETURNS VARCHAR(4000) AS BEGIN RETURN CONVERT(VARCHAR(4000), NULL) END')
GO
ALTER FUNCTION DBO.FN_CERT_LIST_4_REPEX_MOVEMENT_KIROV(
    @ID_SERIES BIGINT
)
...
IF OBJECT_ID('DBO.REPEX_MOVEMENT_KIROV') IS NULL EXEC('CREATE PROCEDURE DBO.REPEX_MOVEMENT_KIROV AS RETURN')
GO
ALTER PROCEDURE DBO.REPEX_MOVEMENT_KIROV
	(@XMLPARAM NTEXT) AS
...
SELECT
	...
	S.BEST_BEFORE,
	REG_CERT_NAME = L.INTERNAL_BARCODE,
	FED_CERT_NAME = DBO.FN_CERT_LIST_4_REPEX_MOVEMENT_KIROV(S.ID_SERIES)
...
--exec REPEX_MOVEMENT_KIROV N'<XML><ID_MOVEMENT>131</ID_MOVEMENT></XML>'