...
DECLARE @ORDER_BY NVARCHAR(256), @SQL NVARCHAR(4000)
...
SELECT
    ...
    II_RETAILPRICE = ISNULL((
        ...
        WHERE L1.ID_GOODS = G.ID_GOODS)),
    OG_SUMQTY =
        ISNULL(SUM(CASE WHEN LM.QUANTITY_SUB < 0 THEN 0.00 ELSE CAST(LM.QUANTITY_SUB AS DECIMAL(18, 2)) END), 0) 
        - (
        ISNULL(SUM(CASE WHEN LM.QUANTITY_SUB > 0 THEN 0.00 ELSE -1 * CAST(LM.QUANTITY_SUB AS DECIMAL(18, 2)) END), 0) 
        +
        ISNULL(SUM(CASE WHEN LM.ID_TABLE = 12 THEN CAST(LM.QUANTITY_ADD AS DECIMAL(18, 2)) ELSE 0.00 END), 0)),
    OG_AVGQTY = CAST(NULL AS DECIMAL(18 , 2)) ,
    LM_STOCKQTY_FROM = (
        SELECT SUM(ISNULL(CAST(LM1.QUANTITY_ADD AS DECIMAL(18 , 2)) , 0) - ISNULL(CAST(LM1.QUANTITY_SUB AS DECIMAL(18 , 2)) , 0)) 
        FROM LOT L1(NOLOCK) INNER JOIN LOT_MOVEMENT LM1(NOLOCK) ON L1.ID_LOT_GLOBAL = LM1.ID_LOT_GLOBAL
...
* 
FROM #All 
WHERE G_DIFFQTY > 0 AND G_FACTDAYS < ' + CAST(@DOC_DAYS AS VARCHAR) + 
CASE WHEN @ORDER_BY IS NOT NULL AND @ORDER_BY != '' THEN ' ORDER BY ' + @ORDER_BY 
--' ORDER BY ' + CASE WHEN ISNULL(@ORDER_BY,0) IS NULL THEN  G_RUSNAME END, LM_STOCKQTY_TO
        ELSE '' END
