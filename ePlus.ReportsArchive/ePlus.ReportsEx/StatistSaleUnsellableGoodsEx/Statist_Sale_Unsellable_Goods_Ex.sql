...
--		ID_LOT_MOVEMENT = MAX(LM.ID_LOT_MOVEMENT),
        ...
        G_QTY_INV = SUM((CASE WHEN LM.CODE_OP in ('INVOICE','IMPORT_REMAINS','INVENTORY_SVED','MOVE') and LM.QUANTITY_ADD>0 THEN LM.QUANTITY_ADD ELSE 0 END)* SR.NUMERATOR / CONVERT(MONEY, SR.DENOMINATOR)),--MAX(ISNULL(MOV.QTY,0)),         --кол-во по приходу за период
...
--		PRICE_SAL = MAX(B.PRICE_SAL), --средняя цена партии за период
        ...
        G_OST_TO = MAX(ISNULL(OST.QTY,0))+
                   ...
                   (SUM((CASE WHEN LM.QUANTITY_SUB>0 THEN LM.QUANTITY_SUB ELSE 0 END)*SR.NUMERATOR / CONVERT(MONEY, SR.DENOMINATOR)) - 
                        SUM((CASE WHEN LM.CODE_OP='ACT_R2B' AND LM.QUANTITY_ADD>0 THEN LM.QUANTITY_ADD ELSE 0 END) * SR.NUMERATOR / CONVERT(MONEY, SR.DENOMINATOR))), --конечный остаток 
...
--    INSERT INTO #TABLE_DATA_SORT(G_ID,G_RUSNAME,G_SUPPLIER,G_QTYSALE,G_QTYRETURN,G_QTYOUT,PRICE_SAL,G_SUMOUT,G_SUMDISCOUNT,G_SUMADD,G_PERCENTADDSUM,G_PERCENTADD,G_PERCENTSUMOUT,G_PERCENT) SELECT G_ID,G_RUSNAME,G_SUPPLIER,G_QTYSALE,G_QTYRETURN,G_QTYOUT,PRICE_SAL,G_SUMOUT,G_SUMDISCOUNT,G_SUMADD,G_PERCENTADDSUM,G_PERCENTADD,G_PERCENTSUMOUT,G_PERCENT FROM #TABLE_DATA ORDER BY G_PERCENT ASC
    	...
    	SET @SQL = 'INSERT INTO #TABLE_DATA_SORT(G_ID,G_RUSNAME,G_CODE,G_SUPPLIER,G_OST_FROM,G_OST_TO,PRICE_SUP,G_QTY_INV,G_QTYSALE,G_QTYRETURN,G_QTYOUT,PRICE_SAL,G_SUMOUT,G_SUMDISCOUNT,G_SUMADD,G_PERCENTADDSUM,G_PERCENTADD,G_PERCENTSUMOUT,G_PERCENT) 
                    SELECT G_ID,G_RUSNAME,G_CODE,G_SUPPLIER,G_OST_FROM,G_OST_TO,PRICE_SUP,G_QTY_INV,G_QTYSALE,G_QTYRETURN,G_QTYOUT,PRICE_SAL,G_SUMOUT,G_SUMDISCOUNT,G_SUMADD,G_PERCENTADDSUM,G_PERCENTADD,G_PERCENTSUMOUT,G_PERCENT FROM #TABLE_DATA ORDER BY CASE WHEN CONVERT(INT,'+ CONVERT(VARCHAR,@QUANTITY) + ')=0 THEN G_PERCENT ELSE G_QTYSALE END ASC'--
    	EXEC(@SQL)
