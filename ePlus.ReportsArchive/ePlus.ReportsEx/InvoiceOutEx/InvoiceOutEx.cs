...
namespace InvoiceOutEx
{
	public class InvoiceOutEx : AbstractDocumentReport, IExternalDocumentPrintForm
	{
		...
		{
			Stream s = this.GetType().Assembly.GetManifestResourceStream("InvoiceOutEx.InvoiceOut.sql");
		...
		void ExtractReport()
		{
			...
			Stream s = this.GetType().Assembly.GetManifestResourceStream("InvoiceOutEx.InvoiceOut.rdlc");
		...
		{
			get { return "Расходные документы: Накладная"; }
		}
	}
}
