...
--USE EPLUS_DEV10
-------------------------------------------------------------------------------------------
IF OBJECT_ID('REP_ACT_RETURNFROMBUYER_WITH_NDS') IS NULL EXEC('CREATE PROCEDURE REP_ACT_RETURNFROMBUYER_WITH_NDS AS RETURN')
GO		
ALTER PROCEDURE REP_ACT_RETURNFROMBUYER_WITH_NDS
	@XMLPARAM NTEXT AS
...
SELECT TOP 1 AB_NUMBER = AB.MNEMOCODE ,
	...
	AB_ADDRESSCONTRACTOR = (SELECT TOP 1 C.ADDRESS FROM CONTRACTOR C(NOLOCK) WHERE C.ID_CONTRACTOR = (SELECT ISNULL(MAX(S.ID_CONTRACTOR) , -1) FROM STORE S(NOLOCK) WHERE S.ID_STORE = AB.ID_STORE_TO))
...
SELECT 
	(CASE WHEN ABI.HAS_LOT = 0 THEN (SELECT ISNULL(MAX(CAST(TT.TAX_RATE AS DECIMAL(18 , 2))) , 0) FROM TAX_TYPE TT(NOLOCK) WHERE TT.ID_TAX_TYPE = ABI.ID_SUPPLIER_TAX_TYPE) ELSE (SELECT ISNULL(MAX(CAST(L.VAT_SUP AS DECIMAL(18 , 2))), 0) FROM LOT L(NOLOCK) WHERE L.ID_LOT_GLOBAL = ABI.ID_LOT_GLOBAL) END) as GROUP_SUPPLIERTAXRATE,
--	(CASE WHEN ABI.HAS_LOT = 0 THEN (SELECT ISNULL(MAX(TT.NAME) , 0) FROM TAX_TYPE TT(NOLOCK) WHERE TT.ID_TAX_TYPE = ABI.ID_SUPPLIER_TAX_TYPE) ELSE (SELECT ISNULL(MAX(CAST(L.VAT_SUP AS DECIMAL(18 , 2))), 0) FROM LOT L(NOLOCK) WHERE L.ID_LOT_GLOBAL = ABI.ID_LOT_GLOBAL) END) as GROUP_SUPPLIERTAXRATE,
	(CASE WHEN ABI.HAS_LOT = 0 THEN 
		ISNULL(CAST(ABI.PRICE_PER_UNIT AS DECIMAL(18 , 2)) , 0) 
		ELSE 
		(SELECT ISNULL(CAST(L.PRICE_SAL AS DECIMAL(18 , 2)) , 0) 
		 FROM LOT L(NOLOCK) 
		 WHERE L.ID_LOT_GLOBAL = ABI.ID_LOT_GLOBAL) 
		END)* 
		(1-100/(100+CASE WHEN ABI.HAS_LOT = 0 THEN (SELECT ISNULL(MAX(CAST(TT.TAX_RATE AS DECIMAL(18 , 2))) , 0) FROM TAX_TYPE TT(NOLOCK) WHERE TT.ID_TAX_TYPE = ABI.ID_SUPPLIER_TAX_TYPE) ELSE (SELECT ISNULL(MAX(CAST(L.VAT_SUP AS DECIMAL(18 , 2))), 0) FROM LOT L(NOLOCK) WHERE L.ID_LOT_GLOBAL = ABI.ID_LOT_GLOBAL) END
		))*ABI.QUANTITY AS GROUP_SUMMNDS
INTO #temp
FROM ACT_RETURN_TO_BUYER_ITEM ABI(NOLOCK)
WHERE	ABI.ID_ACT_RETURN_TO_BUYER = @ID_ACT_RETURN_TO_BUYER
	OR ABI.ID_ACT_RETURN_TO_BUYER_GLOBAL = @ID_ACT_RETURN_TO_BUYER_GLOBAL
SELECT SUM(GROUP_SUMMNDS) as GROUP_SUMMNDS,  REPLACE(CONVERT(VARCHAR(1000),GROUP_SUPPLIERTAXRATE),'.00','%') as GROUP_SUPPLIERTAXRATE from #temp GROUP BY GROUP_SUPPLIERTAXRATE
RETURN 0
GO
-------------------------------------------------------------------------------------------
--EXEC	REP_ACT_RETURNFROMBUYER_WITH_NDS '<XML><ID_ACT_RETURN_TO_BUYER>115</ID_ACT_RETURN_TO_BUYER><ID_ACT_RETURN_TO_BUYER_GLOBAL>53D2D805-9919-45A9-B664-0E5A37F272D4</ID_ACT_RETURN_TO_BUYER_GLOBAL></XML>'
--GO
--exec rep_act_returnfrombuyer @xmlParam = N'<XML><ID_ACT_RETURN_TO_BUYER>142</ID_ACT_RETURN_TO_BUYER></XML>'
--EXEC	REP_ACT_RETURNFROMBUYER_WITH_NDS '<XML><ID_ACT_RETURN_TO_BUYER>141</ID_ACT_RETURN_TO_BUYER><ID_ACT_RETURN_TO_BUYER_GLOBAL>53D2D805-9919-45A9-B664-0E5A37F272D4</ID_ACT_RETURN_TO_BUYER_GLOBAL></XML>'
/*
exec REP_ACT_RETURNFROMBUYER_WITH_NDS N'<XML>
...
EXEC DBO.REMOVE_REPORT_BY_TYPE_NAME 'ReturnFromBuyerWithNDS.ReturnFromBuyer'