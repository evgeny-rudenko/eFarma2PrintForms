...
namespace FCSReturnFromBuyerWithoutNDS
{
    public class ReturnFromBuyer : AbstractDocumentReport, IExternalDocumentPrintForm
    {
        ...
        {
            Stream s = this.GetType().Assembly.GetManifestResourceStream("FCSReturnFromBuyerWithoutNDS.ReturnFromBuyer.sql");
        ...
        private void ExtractReport()
        {
            ...
            Stream s = this.GetType().Assembly.GetManifestResourceStream("FCSReturnFromBuyerWithoutNDS.ReturnFromBuyer.rdlc");
            StreamReader sr = new StreamReader(s);
            string rep = sr.ReadToEnd();
            string reportPath = Path.Combine(cachePath, "ReturnFromBuyer.rdlc");
        ...
        public override IReportForm GetReportForm(DataRowItem dataRowItem)
        {
            ...
            rep.LoadData("REP_RETURNFROMBUYERWITHOUTNDS", doc.InnerXml);
            ...
            string file = Path.Combine(cachePath, "ReturnFromBuyer.rdlc");
        ...
        {
			get { return "Акты возврата от покупателя: Возвратная накладная от покупателя (без НДС)"; }
        }
