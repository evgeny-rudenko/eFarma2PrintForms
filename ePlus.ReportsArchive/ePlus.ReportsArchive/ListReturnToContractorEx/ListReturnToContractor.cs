...
namespace ListReturnToContractorEx
{
	public class ListReturnToContractor : AbstractDocumentReport, IExternalDocumentPrintForm
	{
		...
		void ExtractReport()
		{
			...
			Stream s = this.GetType().Assembly.GetManifestResourceStream("ListReturnToContractorEx.ListReturnToContractor.rdlc");
			using (StreamReader sr = new StreamReader(s))
			{
				using (StreamWriter sw = new StreamWriter(Path.Combine(cachePath, "ListReturnToContractor.rdlc")))
				{
		...
		public override IReportForm GetReportForm(DataRowItem dataRowItem)
		{
			...
			rep.ReportPath = Path.Combine(Path.Combine(folderPath, CACHE_FOLDER), "ListReturnToContractor.rdlc");

			rep.LoadData("REPEX_LIST_RETURN_TO_CONTRACTOR", doc.InnerXml);
			rep.BindDataSource("List_ReturnToContractor_DS_Table", 0);
			rep.BindDataSource("List_ReturnToContractor_DS_Table1", 1);
			rep.BindDataSource("List_ReturnToContractor_DS_Table2", 2);
