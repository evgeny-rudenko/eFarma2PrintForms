...
namespace InvoiceOutPTORG12Ex
{
	public class InvoiceOutPTORG12Ex : AbstractDocumentReport, IExternalDocumentPrintForm
	{
		...
		{
			Stream s = this.GetType().Assembly.GetManifestResourceStream("InvoiceOutPTORG12Ex.TORG12.sql");
		...
		public override IReportForm GetReportForm(DataRowItem dataRowItem)
		{
			...
			using (SqlDataAdapter sqlda = new SqlDataAdapter("REPEX_INVOICE_OUT_PTORG12", connectionString))
			{
			...
			string countInText = ePlus.CommonEx.Reporting.RusCurrency.Str(ds.Tables[0].Rows.Count, "NUM");
			...
			PTORG12 report = new PTORG12();
			ReportFormCrystal reportForm = new ReportFormCrystal();
			reportForm.SetDataSource("Расходная накладная: ТОРГ-12 (альбом.)", ds, report);
			return reportForm;
		}
		...
		{
			get { return "Расходная накладная: ТОРГ-12 (альбом.)"; }
		}
	}
}
