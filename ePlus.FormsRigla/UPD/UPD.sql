...
IF (OBJECT_ID('DBO.REPEX_INVOICE_OUT_INVOICE_RIGLA_UPD') IS NULL) EXEC ('CREATE PROCEDURE DBO.REPEX_INVOICE_OUT_INVOICE_RIGLA_UPD AS RETURN')
GO
ALTER PROCEDURE DBO.REPEX_INVOICE_OUT_INVOICE_RIGLA_UPD
	@XMLPARAM NTEXT AS
...
SELECT
	...
	[CONTRACTOR_FROM_NAME_SHORT] = ISNULL(C_FR.LEGAL_PERS_SHORT,''),
...
--	[GCONTRACTOR] = ISNULL(ISNULL(C_FR.LEGAL_PERS + ' ', C_FR.LEGAL_PERS_SHORT + ' '), '') + ISNULL(C_FR.NAME, 'Не указано') + ', ' + ISNULL(C_FR.ADDRESS, 'Адрес не указан'), 
	...
	[DIRECTOR_FIO] = C_FR.DIRECTOR_FIO,
	[BUH_FIO] = C_FR.BUH_FIO
FROM INVOICE_OUT I
