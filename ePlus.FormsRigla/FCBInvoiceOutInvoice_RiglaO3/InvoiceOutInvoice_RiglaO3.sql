...
--	[GCONTRACTOR] = ISNULL(ISNULL(C_FR.LEGAL_PERS + ' ', C_FR.LEGAL_PERS_SHORT + ' '), '') + ISNULL(C_FR.NAME, 'Не указано') + ', ' + ISNULL(C_FR.ADDRESS, 'Адрес не указан'), 
	...
	[GCONTRACTOR_TO_NAME] = ISNULL(C_TO.NAME+', ','') + ISNULL(C_TO.ADDRESS, ''),
	[CONTRACTOR_TO_NAME] = ISNULL(C_P.NAME,''),
...
--EXEC DBO.REPEX_INVOICE_OUT_INVOICE_RIGLA_O3 N'<XML><ID_GLOBAL>56896F4D-C561-4349-B294-B556E7AFAF9A</ID_GLOBAL></XML>'
...
EXEC DBO.REMOVE_REPORT_BY_TYPE_NAME 'InvoiceOutInvoice_RiglaO3.InvoiceOutInvoice_RiglaO3'