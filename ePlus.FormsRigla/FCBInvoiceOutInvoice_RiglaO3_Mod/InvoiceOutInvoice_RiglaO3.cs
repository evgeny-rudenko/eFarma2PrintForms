...
namespace FCChInvoiceOutInvoice_RiglaO3_Mod
{
  public class InvoiceOutInvoice_RiglaO3 : AbstractDocumentReport, IExternalDocumentPrintForm
  {
    ...
    {
        Stream s = this.GetType().Assembly.GetManifestResourceStream("FCChInvoiceOutInvoice_RiglaO3_Mod.InvoiceOutInvoice_RiglaO3.sql");
    ...
    Stream s = this.GetType().Assembly.GetManifestResourceStream("FCChInvoiceOutInvoice_RiglaO3_Mod.InvoiceOutInvoice_RiglaO3.rdlc");
      using (StreamReader sr = new StreamReader(s))
      {
        using (StreamWriter sw = new StreamWriter(Path.Combine(cachePath, "InvoiceOutInvoice_RiglaO3.rdlc")))
        {
    ...
    public override IReportForm GetReportForm(DataRowItem dataRowItem)
    {
      ...
      rep.ReportPath = Path.Combine(Path.Combine(folderPath, CACHE_FOLDER), "InvoiceOutInvoice_RiglaO3.rdlc");

      rep.LoadData("DBO.REPEX_INVOICE_OUT_INVOICE_RIGLA_O3", doc.InnerXml);
      rep.BindDataSource("FCChInvoiceOutInvoice_RiglaO3_DS_Table0", 0);
      rep.BindDataSource("FCChInvoiceOutInvoice_RiglaO3_DS_Table1", 1);
      rep.BindDataSource("FCChInvoiceOutInvoice_RiglaO3_DS_Table2", 2);
      ...
      using (InvoiceForm paramForm = new InvoiceForm())
      {
        ...
        {
            paramForm.DoverBuh = DoverBuh = Utils.GetString(row, "BUHDOG");
            paramForm.DoverDir = DoverDir = Utils.GetString(row, "DIRDOG");
    ...
    {
      get { return "—чет-фактура O3 (–игла) мод."; }
    }
  }
}
