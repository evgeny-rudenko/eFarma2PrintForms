...
namespace InvoiceRating_Rigla
{
	public class InvoiceRating : AbstractDocumentReport, IExternalDocumentPrintForm
	{
		...
		{
      Stream s = this.GetType().Assembly.GetManifestResourceStream("InvoiceRating_Rigla.InvoiceRating.sql");
      ...
      Stream s = this.GetType().Assembly.GetManifestResourceStream("InvoiceRating_Rigla.InvoiceRating_Rigla.rdlc");
			using (StreamReader sr = new StreamReader(s))
			{
        using (StreamWriter sw = new StreamWriter(Path.Combine(cachePath, "InvoiceRating_Rigla.rdlc")))
				{
      ...
      rep.ReportPath = Path.Combine(Path.Combine(folderPath, CACHE_FOLDER), "InvoiceRating_Rigla.rdlc");

			rep.LoadData("REPEX_INVOICE_RATING_RIGLA", doc.InnerXml);
		...
		{
			get { return "Расценочная накладная (Ригла)"; }
		}
	}
}
