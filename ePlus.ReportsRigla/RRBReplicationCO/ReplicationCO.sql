...
GO
------------------------------------------------------------------------------------------------------------------------------------
IF OBJECT_ID(N'DBO.FN_REPEX_REPLICATION_CO_GETLOG_MAX_DATE', N'FN') IS NOT NULL 
	DROP FUNCTION DBO.FN_REPEX_REPLICATION_CO_GETLOG_MAX_DATE
GO
CREATE FUNCTION DBO.FN_REPEX_REPLICATION_CO_GETLOG_MAX_DATE
...
IF OBJECT_ID('DBO.REPEX_REPLICATION_CO') IS NULL EXEC('CREATE PROCEDURE DBO.REPEX_REPLICATION_CO AS RETURN')
GO
ALTER  PROCEDURE DBO.REPEX_REPLICATION_CO
				@XMLPARAM NTEXT AS
	DECLARE @HDOC INT
	DECLARE @ALL_AU BIT
	DECLARE @CO BIT
	...
	SELECT 
		...
		,DATE_UPLOAD_CO = DBO.FN_REPEX_REPLICATION_CO_GETLOG_MAX_DATE(RS.ID_REPLICATION_SESSION)
		,MSG_TYPE =CASE WHEN RS.SESSION_STATE='IMP_ERR' THEN 'ERR' ELSE CASE WHEN DBO.FN_REPEX_REPLICATION_CO_GETLOG_MAX_DATE(RS.ID_REPLICATION_SESSION) >= @DATE_LATE THEN 'OK' ELSE 'LATE' END END
...
--exec REPEX_REPLICATION_CO @xmlParam=N''
