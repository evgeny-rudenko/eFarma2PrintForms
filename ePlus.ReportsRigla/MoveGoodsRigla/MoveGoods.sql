...
IF OBJECT_ID('DBO.REPEX_MOVE_GOODS_RIGLA') IS NULL BEGIN EXEC('CREATE PROCEDURE DBO.REPEX_MOVE_GOODS_RIGLA AS RETURN') END
GO
ALTER PROCEDURE DBO.REPEX_MOVE_GOODS_RIGLA
    @XMLPARAM NTEXT AS
...
-- актуализировать периодические остатки на конец предыдущего дн€ периода
DECLARE @DATE_BEG_REM DATETIME,
        @TIMING_MODE BIT

SET @TIMING_MODE = 0            -- режим замера скорости выполнени€ запросов

-- SET @DATE_BEG_REM = @DATE_FR - 1        -- остатки на конец предыдущего дн€
...
IF @TIMING_MODE = 1 PRINT 'ѕолучение остатков на начало ' + CAST(DATEDIFF(MS, @S, GETDATE()) AS VARCHAR(20))
SET @S = GETDATE()
...
--
IF @TIMING_MODE = 1 
    PRINT 'ѕолучение движени€ шаг 1 (из LOT_MOVEMENT в #MOV_IN_PERION)  ' + CAST(DATEDIFF(MS, @S, GETDATE()) AS VARCHAR(20))
SET @S = GETDATE()
--
-- строку соединени€ переделать, т.к. в этом запросе соединение с LOT уже есть изначально
SET @SQL_INNER = ''
...
--
IF @TIMING_MODE = 1 
    PRINT 'ѕолучение движени€ шаг 2 (вставка в #MOV_IN_PERIOD_EXT)  ' + CAST(DATEDIFF(MS, @S, GETDATE()) AS VARCHAR(20))
SET @S = GETDATE()

--=================================================================
SET @SQL_WHERE = ''
-- фильтр "не показывать товар без движени€"
IF @ONLY_WITH_MOV = 1 
...
IF @TIMING_MODE = 1
    PRINT 'ѕолучение движени€ и остатков (вставка в #MOV_AND_REM) ' + CAST(DATEDIFF(MS, @S, GETDATE()) AS VARCHAR(20))
SET @S = GETDATE()
-- результирующа€ выборка 
-- отладка
...
WHERE (ISNULL(T.BEGIN_COUNT, 0) <> 0 OR ISNULL(T.INVOICE_COUNT, 0) <> 0 OR ISNULL(T.EXPENSE_COUNT, 0) <> 0)'
 ...
 EXEC SP_EXECUTESQL @SQL
--
IF @TIMING_MODE = 1 PRINT '»тогова€ выборка ' + CAST(DATEDIFF(MS, @S, GETDATE()) AS VARCHAR(20))
--
