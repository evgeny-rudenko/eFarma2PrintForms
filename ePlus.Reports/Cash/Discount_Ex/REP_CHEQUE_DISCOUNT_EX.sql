IF OBJECT_ID('DBO.REP_CHEQUE_DISCOUNT_EX') IS NULL EXEC('CREATE PROCEDURE DBO.REP_CHEQUE_DISCOUNT_EX AS RETURN')
GO
ALTER PROCEDURE REP_CHEQUE_DISCOUNT_EX
     (@XMLPARAM NTEXT) AS
/* PARAMETERS */
...
DECLARE @ALL_DISCOUNT_MEMBER BIT, @ALL_DISCOUNT_CARD_TYPE BIT, @ALL_CASH_REGISTER BIT
DECLARE @WITH_RETURN BIT  --Û˜ËÚ˚‚‡Ú¸ ËÎË ÌÂÚ ‚ÓÁ‚‡Ú˚
...
EXEC SP_XML_PREPAREDOCUMENT @HDOC OUTPUT, @XMLPARAM OUTPUT
	...
	/* “¿¡À»÷¿ »ƒ≈Õ“»‘» ¿“Œ–Œ¬  */
	SELECT * INTO #DISCOUNT2_CARD 
	FROM OPENXML(@HDOC, '//ID_DISCOUNT2_CARD_GLOBAL') 
	WITH(ID_DISCOUNT2_CARD_GLOBAL UNIQUEIDENTIFIER '.')

	IF @@ROWCOUNT = 0 SET @ALL_DISCOUNT_CARD = 1;

	/* “¿¡À»÷¿ »ƒ≈Õ“»‘» ¿“Œ–Œ¬  */
	SELECT * INTO #DISCOUNT2_MEMBER 
	FROM OPENXML(@HDOC, '//ID_DISCOUNT2_MEMBER_GLOBAL') 
	WITH(ID_DISCOUNT2_MEMBER_GLOBAL UNIQUEIDENTIFIER '.')

	IF @@ROWCOUNT = 0 SET @ALL_DISCOUNT_MEMBER = 1;

	/* “¿¡À»÷¿ »ƒ≈Õ“»‘» ¿“Œ–Œ¬  */
	SELECT * INTO #DISCOUNT2_CARD_TYPE 
	FROM OPENXML(@HDOC, '//ID_DISCOUNT2_CARD_TYPE_GLOBAL') 
	WITH(ID_DISCOUNT2_CARD_TYPE_GLOBAL UNIQUEIDENTIFIER '.')
	
	IF @@ROWCOUNT = 0 SET @ALL_DISCOUNT_CARD_TYPE = 1;
	
	/* “¿¡À»÷¿ »ƒ≈Õ“»‘» ¿“Œ–Œ¬  */
	SELECT * INTO #DISCOUNT2_PROGRAM 
	FROM OPENXML(@HDOC, '//ID_DISCOUNT2_PROGRAM_GLOBAL') 
	...
	SELECT
		TYPE = CASE WHEN DMI.ID_DISCOUNT2_CARD_GLOBAL IS NOT NULL THEN 'ƒ : '+ISNULL(DCK.[NAME], '')+'; '+ISNULL(DC.BARCODE,'')+'; '+ISNULL(DM.LASTNAME,'')+
		 	   ...
		 	   ELSE 'ƒœ: '+ISNULL(DP.[NAME], '') 
			   END,--“»œ(Õ¿»Ã≈ÕŒ¬¿Õ»≈)
		DISCOUNT_VALUE = CASE DISCOUNT_TYPE WHEN 'PERCENT' THEN DP.DISCOUNT_VALUE ELSE DMI.AMOUNT*100/(CHI.PRICE*CHI.QUANTITY) END,
		G_CODE = G.CODE,
		VALUE_TEXT = G.[NAME],--Õ¿»Ã≈ÕŒ¬¿Õ»≈ “Œ¬¿–¿
		QUANTITY = CASE WHEN CH.CHEQUE_TYPE = 'RETURN' THEN -1 ELSE CASE WHEN CH.CHEQUE_TYPE = 'SALE' THEN 1 ELSE 0 END END * CHI.QUANTITY * SR.NUMERATOR / CONVERT(MONEY, SR.DENOMINATOR),-- ŒÀ»◊≈—“¬Œ
		UNIT.SHORT_NAME,---≈ƒ. »«Ã.
		SUMM = CASE WHEN CH.CHEQUE_TYPE = 'RETURN' THEN -1 ELSE CASE WHEN CH.CHEQUE_TYPE = 'SALE' THEN 1 ELSE 0 END END * CHI.SUMM,--—”ÃÃ¿ œ–Œƒ¿∆»
		SUMM_DISCOUNT = /* DMI.AMOUNT,-- */CASE WHEN CH.CHEQUE_TYPE = 'RETURN' THEN -1 ELSE CASE WHEN CH.CHEQUE_TYPE = 'SALE' THEN 1 ELSE 0 END END * CHI.SUMM_DISCOUNT,--—”ÃÃ¿ — »ƒ »
		L.ID_STORE, 
		CR.ID_CASH_REGISTER,
		SUMM_BEZ = CASE WHEN CH.CHEQUE_TYPE = 'RETURN' THEN -1 ELSE CASE WHEN CH.CHEQUE_TYPE = 'SALE' THEN 1 ELSE 0 END END * CHI.PRICE*CHI.QUANTITY --—”ÃÃ¿ ¡≈« — »ƒ »
	...
	WHERE CH.DATE_CHEQUE BETWEEN @DATE_FR AND @DATE_TO
		...
		AND (@WITH_RETURN = 1 OR CH.CHEQUE_TYPE <> 'RETURN')
...
/*
exec REP_CHEQUE_DISCOUNT_EX N'
<XML>
	<DATE_FR>2010-05-26T14:02:00.906</DATE_FR>
	<DATE_TO>2010-05-26T14:02:00.921</DATE_TO>
</XML>'*/
...
EXEC DBO.REMOVE_REPORT_BY_TYPE_NAME 'Discount_Ex.FormParams'
