...
*/
select count(distinct convert(char(8), ISNULL(date_close, CH.DATE_CHEQUE), 112)) as PERIOD_DAYS
from cash_session CS
INNER JOIN (select 
				DATE_CHEQUE = MAX(DATE_CHEQUE)
				,ID_CASH_SESSION_GLOBAL
		    from  CHEQUE 
		    GROUP BY ID_CASH_SESSION_GLOBAL
		    ) as CH ON CS.ID_CASH_SESSION_GLOBAL = CH.ID_CASH_SESSION_GLOBAL
where ISNULL(date_close, CH.DATE_CHEQUE) between @date_fr and @date_to
...
<XML>
	<DATE_FR>2011-08-01T00:00:00.546</DATE_FR>
	<DATE_TO>2011-08-20T22:17:39.546</DATE_TO>
...
EXEC DBO.REMOVE_REPORT_BY_TYPE_NAME 'KKMQuantityEx.KKMQuantityParams'