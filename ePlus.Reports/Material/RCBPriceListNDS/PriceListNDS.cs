...
{
  public partial class PriceListNDS : ExternalReportForm, IExternalReportFormMethods
  {
    public PriceListNDS()
    {
      InitializeComponent();
    ...
    public void Print(string[] reportFiles)
    {
      ...
      rep.ReportPath = Path.Combine(Path.GetDirectoryName(reportFiles[0]), "PriceListNDS.rdlc");
      rep.LoadData("REPEX_PRICE_LIST_NDS", doc.InnerXml);
      rep.BindDataSource("RCBPriceListNDS_DS_Table", 0);
    ...
    private void PriceListNDS_FormClosed(object sender, FormClosedEventArgs e)
    {
      SaveSettings();
    }

    private void PriceListNDS_Load(object sender, EventArgs e)
    {
      LoadSettings();
      //если МЫ не является центром, то делаем плагин неактивным
      //ucDrugstores.Enabled = SelfIsCenter();
