...
IF OBJECT_ID('DBO.REPEX_REMAINS_LESS_INS_RESERVE') IS NULL EXEC('CREATE PROCEDURE DBO.REPEX_REMAINS_LESS_INS_RESERVE AS RETURN')
GO
ALTER  PROCEDURE DBO.REPEX_REMAINS_LESS_INS_RESERVE
    @XMLPARAM NTEXT AS
	DECLARE @HDOC INT
	DECLARE @CO BIT
...
SELECT
    ...
    INS_RESERVE = ISNULL(G.QUANTITY_MIN, 0), 
    SR.ID_STORE, 
    SR.ID_CONTRACTOR
...
WHERE 
	...
	AND (G.ID_GOODS IN (SELECT ID_GOODS FROM #ID_GOODS) OR @ALL_GOODS = 1 OR @RESTS_ONLY = 1)
GROUP BY
    G.ID_GOODS,
    G.NAME,
    ISNULL(G.QUANTITY_MIN, 0),  
    SR.ID_STORE, 
    SR.ID_CONTRACTOR
HAVING ISNULL(G.QUANTITY_MIN, 0) > 0
) A
