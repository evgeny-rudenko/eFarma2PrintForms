...
FROM OPENXML(@HDOC, '//XML/DOC_TYPES/DOC_TYPE', 2) WITH(DOC_TYPE VARCHAR(40) '.')
IF (@@ROWCOUNT = 0)
	SET @ALL_TYPES = 1
...
SELECT ID_STORE, ID_CONTRACTOR INTO #STORES_EX FROM @STORES_TO
INSERT INTO #STORES_EX
SELECT ID_STORE, ID_CONTRACTOR FROM STORE WHERE ID_CONTRACTOR NOT IN (SELECT ID_CONTRACTOR FROM @STORES_TO)
...
-- AND
--		(@NOAU = 1 OR S_TO.ID_STORE NOT IN (SELECT ID_STORE FROM #STORES_EX))
END
...
BEGIN
	...
	SELECT
		...
		SUP_SUM = D.SUM_SUP,
		SUP_VAT = D.SVAT_SUP,
		VAT_10 = (SELECT SUM(PVAT_SUP * QUANTITY) FROM INVENTORY_VED_ITEM IVI INNER JOIN INVENTORY_VED IV ON IV.ID_INVENTORY_VED_GLOBAL = IVI.ID_INVENTORY_VED_GLOBAL WHERE IVI.VAT_SUP = 10 AND IV.ID_INVENTORY_SVED_GLOBAL = D.ID_INVENTORY_GLOBAL),
		VAT_18 = (SELECT SUM(PVAT_SUP * QUANTITY) FROM INVENTORY_VED_ITEM IVI INNER JOIN INVENTORY_VED IV ON IV.ID_INVENTORY_VED_GLOBAL = IVI.ID_INVENTORY_VED_GLOBAL WHERE IVI.VAT_SUP = 18 AND IV.ID_INVENTORY_SVED_GLOBAL = D.ID_INVENTORY_GLOBAL)
...
BEGIN
	...
	SELECT
		...
		VAT_10 = (SELECT SUM(SVAT_SUP) FROM VAT_CORRECT_ITEM WHERE VAT_RATE = 10 AND ID_VAT_CORRECT_GLOBAL = D.ID_VAT_CORRECT_GLOBAL),
		VAT_18 = (SELECT SUM(SVAT_SUP) FROM VAT_CORRECT_ITEM WHERE VAT_RATE = 18 AND ID_VAT_CORRECT_GLOBAL = D.ID_VAT_CORRECT_GLOBAL)
	FROM VAT_CORRECT D
		INNER JOIN STORE S_FROM ON S_FROM.ID_STORE = D.ID_STORE
		INNER JOIN CONTRACTOR C_FROM ON C_FROM.ID_CONTRACTOR = S_FROM.ID_CONTRACTOR
	WHERE D.DOC_DATE BETWEEN @DATE_FROM AND @DATE_TO AND
...
IF (@ALL_TYPES = 1 OR EXISTS (SELECT NULL FROM #DOC_TYPES WHERE DOC_TYPE = 'INTERFIRM_MOVING'))
BEGIN
	INSERT INTO @DOCUMENTS
	(
		DOC_DATE,
	...
	WHERE D.DATE BETWEEN @DATE_FROM AND @DATE_TO AND
		...
		(@ALL_STORES_FROM = 1 OR S_FROM.ID_STORE IN (SELECT ID_STORE FROM @STORES_FROM AS ST INNER JOIN CONTRACTOR AS C ON ST.ID_CONTRACTOR = C.ID_CONTRACTOR WHERE ST.ID_CONTRACTOR = C_FROM.ID_CONTRACTOR)) AND
		(@ALL_STORES_TO = 1 OR S_TO.ID_STORE IN (SELECT ID_STORE FROM @STORES_TO AS ST INNER JOIN CONTRACTOR AS C ON ST.ID_CONTRACTOR = C.ID_CONTRACTOR WHERE ST.ID_CONTRACTOR = C_TO.ID_CONTRACTOR)) AND
		(@ALL_GOODS = 1 OR EXISTS (SELECT NULL FROM INTERFIRM_MOVING_ITEM II INNER JOIN LOT L ON L.ID_LOT = II.ID_LOT_TO INNER JOIN GOODS G ON G.ID_GOODS = L.ID_GOODS WHERE II.ID_INTERFIRM_MOVING_GLOBAL = D.ID_INTERFIRM_MOVING_GLOBAL AND EXISTS (SELECT NULL FROM #GOODS GT WHERE GT.ID_GOOD = G.ID_GOODS)))
END

IF (@ALL_TYPES = 1 OR EXISTS (SELECT NULL FROM #DOC_TYPES WHERE DOC_TYPE = 'INTERFIRM_MOVING_ACCEPTANCE_ACT'))
BEGIN
	INSERT INTO @DOCUMENTS
	(
		DOC_DATE,
	...
	WHERE D.DATE BETWEEN @DATE_FROM AND @DATE_TO AND
		...
		(@ALL_STORES_FROM = 1 OR S_FROM.ID_STORE IN (SELECT ID_STORE FROM @STORES_FROM AS ST INNER JOIN CONTRACTOR AS C ON ST.ID_CONTRACTOR = C.ID_CONTRACTOR WHERE ST.ID_CONTRACTOR = C_FROM.ID_CONTRACTOR)) AND
		(@ALL_STORES_TO = 1 OR S_TO.ID_STORE IN (SELECT ID_STORE FROM @STORES_TO AS ST INNER JOIN CONTRACTOR AS C ON ST.ID_CONTRACTOR = C.ID_CONTRACTOR WHERE ST.ID_CONTRACTOR = C_TO.ID_CONTRACTOR)) AND
		(@ALL_GOODS = 1 OR EXISTS (SELECT NULL FROM INTERFIRM_MOVING_ACCEPTANCE_ACT_ITEM II INNER JOIN LOT L ON L.ID_LOT = II.ID_LOT_TO INNER JOIN GOODS G ON G.ID_GOODS = L.ID_GOODS WHERE II.ID_INTERFIRM_MOVING_ACCEPTANCE_ACT_GLOBAL = D.ID_INTERFIRM_MOVING_ACCEPTANCE_ACT_GLOBAL AND EXISTS (SELECT NULL FROM #GOODS GT WHERE GT.ID_GOOD = G.ID_GOODS)))
-- AND
--	(@NOAU = 1 OR S_TO.ID_STORE NOT IN (SELECT ID_STORE FROM #STORES_EX))
END

IF (@ALL_TYPES = 1 OR EXISTS (SELECT NULL FROM #DOC_TYPES WHERE DOC_TYPE = 'INTERFIRM_MOVING_REFUSAL_ACT'))
BEGIN
	INSERT INTO @DOCUMENTS
	(
		DOC_DATE,
	...
	WHERE D.DATE BETWEEN @DATE_FROM AND @DATE_TO AND
		...
		(@ALL_STORES_FROM = 1 OR S_FROM.ID_STORE IN (SELECT ID_STORE FROM @STORES_FROM AS ST INNER JOIN CONTRACTOR AS C ON ST.ID_CONTRACTOR = C.ID_CONTRACTOR WHERE ST.ID_CONTRACTOR = C_FROM.ID_CONTRACTOR)) AND
		(@ALL_STORES_TO = 1 OR S_TO.ID_STORE IN (SELECT ID_STORE FROM @STORES_TO AS ST INNER JOIN CONTRACTOR AS C ON ST.ID_CONTRACTOR = C.ID_CONTRACTOR WHERE ST.ID_CONTRACTOR = C_TO.ID_CONTRACTOR)) AND
		(@ALL_GOODS = 1 OR EXISTS (SELECT NULL FROM INTERFIRM_MOVING_REFUSAL_ACT_ITEM II INNER JOIN LOT L ON L.ID_LOT = II.ID_LOT_TO INNER JOIN GOODS G ON G.ID_GOODS = L.ID_GOODS WHERE II.ID_INTERFIRM_MOVING_REFUSAL_ACT_GLOBAL = D.ID_INTERFIRM_MOVING_REFUSAL_ACT_GLOBAL AND EXISTS (SELECT NULL FROM #GOODS GT WHERE GT.ID_GOOD = G.ID_GOODS)))
END
...
<XML>
	<DATE_FROM>2009-12-01T16:36:25.046</DATE_FROM>
	<DATE_TO>2009-12-29T16:36:25.046</DATE_TO>
	<SORT_DOC>0</SORT_DOC>
	<DOC_TYPES>
		<DOC_TYPE>INTERFIRM_MOVING_REFUSAL_ACT</DOC_TYPE>
	</DOC_TYPES>
	<NOAU>1</NOAU>
</XML>'*/
...
IF (OBJECT_ID(N'DBO.REPEX_DOCS_REGISTRY_SAL') IS NULL) EXEC (N'CREATE PROCEDURE DBO.REPEX_DOCS_REGISTRY_SAL AS RETURN')
GO
ALTER PROCEDURE DBO.REPEX_DOCS_REGISTRY_SAL
    @XMLPARAM NTEXT AS
...
BEGIN	
	...
	SELECT
		...
		SUP_VAT = D.SVAT_SUPPLIER,
		BASE_DOC = D.INCOMING_NUMBER + CASE WHEN D.INCOMING_DATE IS NULL THEN '' ELSE ' от ' + CONVERT(VARCHAR, D.INCOMING_DATE, 104) END,
		VAT_10 = (SELECT SUM(LM.SVAT_ACC) FROM LOT_MOVEMENT LM INNER JOIN LOT L ON L.ID_LOT_GLOBAL = LM.ID_LOT_GLOBAL WHERE L.VAT_SAL = 10 AND LM.ID_DOCUMENT = D.ID_INVOICE_GLOBAL),
		VAT_18 = (SELECT SUM(LM.SVAT_ACC) FROM LOT_MOVEMENT LM INNER JOIN LOT L ON L.ID_LOT_GLOBAL = LM.ID_LOT_GLOBAL WHERE L.VAT_SAL = 18 AND LM.ID_DOCUMENT = D.ID_INVOICE_GLOBAL)
	FROM INVOICE D
		INNER JOIN CONTRACTOR C_FROM ON C_FROM.ID_CONTRACTOR = D.ID_CONTRACTOR_SUPPLIER
		INNER JOIN STORE S_TO ON S_TO.ID_STORE = D.ID_STORE
		INNER JOIN CONTRACTOR C_TO ON C_TO.ID_CONTRACTOR = S_TO.ID_CONTRACTOR
...
BEGIN
	...
	SELECT
		...
		SUP_VAT = (SELECT SUM((I.PRICE_VAT - I.PRICE) * I.QUANTITY) FROM ACT_RETURN_TO_CONTRACTOR_ITEM I WHERE I.ID_ACT_RETURN_TO_CONTRACTOR_GLOBAL = D.ID_ACT_RETURN_TO_CONTRACTOR_GLOBAL),
		BASE_DOC = D.BASE_DOCUMENT_NAME,
		VAT_10 = (SELECT SUM(LM.SVAT_ACC) FROM LOT_MOVEMENT LM INNER JOIN LOT L ON L.ID_LOT_GLOBAL = LM.ID_LOT_GLOBAL WHERE L.VAT_SAL = 10 AND LM.ID_DOCUMENT = D.ID_ACT_RETURN_TO_CONTRACTOR_GLOBAL),
		VAT_18 = (SELECT SUM(LM.SVAT_ACC) FROM LOT_MOVEMENT LM INNER JOIN LOT L ON L.ID_LOT_GLOBAL = LM.ID_LOT_GLOBAL WHERE L.VAT_SAL = 18 AND LM.ID_DOCUMENT = D.ID_ACT_RETURN_TO_CONTRACTOR_GLOBAL)
	FROM ACT_RETURN_TO_CONTRACTOR D
		INNER JOIN CONTRACTOR C_TO ON C_TO.ID_CONTRACTOR = D.ID_CONTRACTOR_TO
		INNER JOIN STORE S_FROM ON S_FROM.ID_STORE = D.ID_STORE
		INNER JOIN CONTRACTOR C_FROM ON C_FROM.ID_CONTRACTOR = S_FROM.ID_CONTRACTOR
...
BEGIN
	...
	SELECT
		...
		SUP_VAT = (SELECT SUM(L.PVAT_SUP * DI.QUANTITY_FROM) FROM LOT L INNER JOIN ACT_DISASSEMBLING_ITEM DI ON DI.ID_LOT_FROM = L.ID_LOT WHERE DI.ID_ACT_DISASSEMBLING_GLOBAL = D.ID_ACT_DISASSEMBLING_GLOBAL),
		BASE_DOC = D.BASE_DOCUMENT_NAME,
		VAT_10 = (SELECT SUM(LM.SVAT_ACC) FROM LOT_MOVEMENT LM INNER JOIN LOT L ON L.ID_LOT_GLOBAL = LM.ID_LOT_GLOBAL WHERE L.VAT_SAL = 10 AND LM.QUANTITY_ADD > 0 AND LM.ID_DOCUMENT = D.ID_ACT_DISASSEMBLING_GLOBAL),
		VAT_18 = (SELECT SUM(LM.SVAT_ACC) FROM LOT_MOVEMENT LM INNER JOIN LOT L ON L.ID_LOT_GLOBAL = LM.ID_LOT_GLOBAL WHERE L.VAT_SAL = 18 AND LM.QUANTITY_ADD > 0 AND LM.ID_DOCUMENT = D.ID_ACT_DISASSEMBLING_GLOBAL)
	FROM ACT_DISASSEMBLING D
		INNER JOIN STORE S_FROM ON S_FROM.ID_STORE = D.ID_STORE
		INNER JOIN CONTRACTOR C_FROM ON C_FROM.ID_CONTRACTOR = S_FROM.ID_CONTRACTOR
	WHERE D.DATE BETWEEN @DATE_FROM AND @DATE_TO AND
...
BEGIN
	...
	SELECT
		...
		SUP_SUM = (SELECT SUM(L.PRICE_SUP * DI.QUANTITY) FROM LOT L INNER JOIN CHEQUE_ITEM DI ON DI.ID_LOT_GLOBAL = L.ID_LOT_GLOBAL WHERE DI.ID_CHEQUE_GLOBAL = D.ID_CHEQUE_GLOBAL),
		SUP_VAT = (SELECT SUM(L.PVAT_SUP * DI.QUANTITY) FROM LOT L INNER JOIN CHEQUE_ITEM DI ON DI.ID_LOT_GLOBAL = L.ID_LOT_GLOBAL WHERE DI.ID_CHEQUE_GLOBAL = D.ID_CHEQUE_GLOBAL),
		VAT_10 = (SELECT SUM(L.PVAT_SAL * DI.QUANTITY) FROM LOT L INNER JOIN CHEQUE_ITEM DI ON DI.ID_LOT_GLOBAL = L.ID_LOT_GLOBAL WHERE DI.ID_CHEQUE_GLOBAL = D.ID_CHEQUE_GLOBAL AND L.VAT_SAL = 10),
		VAT_18 = (SELECT SUM(L.PVAT_SAL * DI.QUANTITY) FROM LOT L INNER JOIN CHEQUE_ITEM DI ON DI.ID_LOT_GLOBAL = L.ID_LOT_GLOBAL WHERE DI.ID_CHEQUE_GLOBAL = D.ID_CHEQUE_GLOBAL AND L.VAT_SAL = 18)
	FROM CHEQUE D
		INNER JOIN CASH_SESSION CS ON CS.ID_CASH_SESSION_GLOBAL = D.ID_CASH_SESSION_GLOBAL
		INNER JOIN CASH_REGISTER CR ON CR.ID_CASH_REGISTER = CS.ID_CASH_REGISTER
		INNER JOIN CONTRACTOR C_FROM ON C_FROM.ID_CONTRACTOR = CR.ID_CONTRACTOR
...
BEGIN
	...
	SELECT
		...
		SUP_VAT = D.TOTAL_SUPPLIER_VAT - D.TOTAL_SUPPLIER,
		BASE_DOC = D.BASE_DOCUMENT_NAME,
		VAT_10 = (SELECT SUM(LM.SVAT_ACC) FROM LOT_MOVEMENT LM INNER JOIN LOT L ON L.ID_LOT_GLOBAL = LM.ID_LOT_GLOBAL WHERE L.VAT_SAL = 10 AND D.ID_STORE_TO = L.ID_STORE AND LM.ID_DOCUMENT = D.ID_MOVEMENT_GLOBAL),
		VAT_18 = (SELECT SUM(LM.SVAT_ACC) FROM LOT_MOVEMENT LM INNER JOIN LOT L ON L.ID_LOT_GLOBAL = LM.ID_LOT_GLOBAL WHERE L.VAT_SAL = 18 AND D.ID_STORE_TO = L.ID_STORE AND LM.ID_DOCUMENT = D.ID_MOVEMENT_GLOBAL)
	FROM MOVEMENT D
		INNER JOIN STORE S_FROM ON S_FROM.ID_STORE = D.ID_STORE_FROM
		INNER JOIN CONTRACTOR C_FROM ON C_FROM.ID_CONTRACTOR = S_FROM.ID_CONTRACTOR
		INNER JOIN STORE S_TO ON S_TO.ID_STORE = D.ID_STORE_TO
...
BEGIN
	...
	SELECT
		...
		SUP_VAT = D.TOTAL_WITH_VAT_SUPPLIER - D.TOTAL_SUPPLIER,
		BASE_DOC = CASE WHEN D.ID_TABLE_DATA_DOCUMENT_BASE = 21 THEN 'Расходная накладная ' + (SELECT MNEMOCODE + ' от ' + CONVERT(VARCHAR, [DATE], 104) FROM INVOICE_OUT WHERE ID_INVOICE_OUT = D.ID_DOCUMENT_BASE) WHEN D.ID_TABLE_DATA_DOCUMENT_BASE = 7 THEN 'Чек ' + (SELECT MNEMOCODE + ' от ' + CONVERT(VARCHAR, DATE_CHEQUE, 104) FROM CHEQUE WHERE ID_CHEQUE = D.ID_DOCUMENT_BASE) END,
		VAT_10 = (SELECT SUM(LM.SVAT_ACC) FROM LOT_MOVEMENT LM INNER JOIN LOT L ON L.ID_LOT_GLOBAL = LM.ID_LOT_GLOBAL WHERE L.VAT_SAL = 10 AND LM.ID_DOCUMENT = D.ID_ACT_RETURN_TO_BUYER_GLOBAL),
		VAT_18 = (SELECT SUM(LM.SVAT_ACC) FROM LOT_MOVEMENT LM INNER JOIN LOT L ON L.ID_LOT_GLOBAL = LM.ID_LOT_GLOBAL WHERE L.VAT_SAL = 18 AND LM.ID_DOCUMENT = D.ID_ACT_RETURN_TO_BUYER_GLOBAL)
	FROM ACT_RETURN_TO_BUYER D
		INNER JOIN CONTRACTOR C_FROM ON C_FROM.ID_CONTRACTOR = D.ID_CONTRACTOR_FROM
		INNER JOIN STORE S_TO ON S_TO.ID_STORE = D.ID_STORE_TO
		INNER JOIN CONTRACTOR C_TO ON C_TO.ID_CONTRACTOR = S_TO.ID_CONTRACTOR
...
BEGIN
	...
	SELECT
		...
		SUP_VAT = D.SVAT_SUP,
		BASE_DOC = D.BASE_DOC_TEXT,
		VAT_10 = (SELECT SUM(LM.SVAT_ACC) FROM LOT_MOVEMENT LM INNER JOIN LOT L ON L.ID_LOT_GLOBAL = LM.ID_LOT_GLOBAL WHERE L.VAT_SAL = 10 AND LM.QUANTITY_ADD > 0 AND LM.ID_DOCUMENT = D.ID_ACT_REVALUATION2_GLOBAL),
		VAT_18 = (SELECT SUM(LM.SVAT_ACC) FROM LOT_MOVEMENT LM INNER JOIN LOT L ON L.ID_LOT_GLOBAL = LM.ID_LOT_GLOBAL WHERE L.VAT_SAL = 18 AND LM.QUANTITY_ADD > 0 AND LM.ID_DOCUMENT = D.ID_ACT_REVALUATION2_GLOBAL)
	FROM ACT_REVALUATION2 D
        INNER JOIN STORE S_FROM ON S_FROM.ID_STORE = D.ID_STORE
        INNER JOIN CONTRACTOR C_FROM ON C_FROM.ID_CONTRACTOR = S_FROM.ID_CONTRACTOR
	WHERE D.DATE BETWEEN @DATE_FROM AND @DATE_TO AND
...
IF (@ALL_TYPES = 1 OR EXISTS (SELECT NULL FROM #DOC_TYPES WHERE DOC_TYPE = 'ACT_DEDUCTION')) BEGIN
	...
	SELECT
		...
		SUP_VAT = (SELECT SUM(L.PVAT_SUP * DI.QUANTITY) FROM LOT L INNER JOIN ACT_DEDUCTION_ITEM DI ON DI.ID_LOT_GLOBAL = L.ID_LOT_GLOBAL WHERE DI.ID_ACT_DEDUCTION_GLOBAL = D.ID_ACT_DEDUCTION_GLOBAL),
		BASE_DOC = D.BASE_DOCUMENT_NAME,
		VAT_10 = (SELECT SUM(LM.SVAT_ACC) FROM LOT_MOVEMENT LM INNER JOIN LOT L ON L.ID_LOT_GLOBAL = LM.ID_LOT_GLOBAL WHERE L.VAT_SAL = 10 AND LM.ID_DOCUMENT = D.ID_ACT_DEDUCTION_GLOBAL),
		VAT_18 = (SELECT SUM(LM.SVAT_ACC) FROM LOT_MOVEMENT LM INNER JOIN LOT L ON L.ID_LOT_GLOBAL = LM.ID_LOT_GLOBAL WHERE L.VAT_SAL = 18 AND LM.ID_DOCUMENT = D.ID_ACT_DEDUCTION_GLOBAL)
	FROM ACT_DEDUCTION D
		INNER JOIN STORE S_FROM ON S_FROM.ID_STORE = D.ID_STORE
		INNER JOIN CONTRACTOR C_FROM ON C_FROM.ID_CONTRACTOR = S_FROM.ID_CONTRACTOR
	WHERE D.DATE BETWEEN @DATE_FROM AND @DATE_TO AND
...
BEGIN
	...
	SELECT
		...
		SUP_VAT = D.SUM_SUP - D.SVAT_SUP,
		BASE_DOC = D.BASE_DOCUMENT_NAME,
		VAT_10 = (SELECT SUM(LM.SVAT_ACC) FROM LOT_MOVEMENT LM INNER JOIN LOT L ON L.ID_LOT_GLOBAL = LM.ID_LOT_GLOBAL WHERE L.VAT_SAL = 10 AND LM.ID_DOCUMENT = D.ID_INVOICE_OUT_GLOBAL),
		VAT_18 = (SELECT SUM(LM.SVAT_ACC) FROM LOT_MOVEMENT LM INNER JOIN LOT L ON L.ID_LOT_GLOBAL = LM.ID_LOT_GLOBAL WHERE L.VAT_SAL = 18 AND LM.ID_DOCUMENT = D.ID_INVOICE_OUT_GLOBAL)
	FROM INVOICE_OUT D
		INNER JOIN STORE S_FROM ON S_FROM.ID_STORE = D.ID_STORE
		INNER JOIN CONTRACTOR C_FROM ON C_FROM.ID_CONTRACTOR = S_FROM.ID_CONTRACTOR
		INNER JOIN CONTRACTOR C_TO ON C_TO.ID_CONTRACTOR = D.ID_CONTRACTOR_TO
...
BEGIN
	...
	SELECT
		...
		SUP_SUM = (SELECT SUM(L.PRICE_SUP * DI.QUANTITY) FROM BILL_ITEM DI INNER JOIN LOT L ON L.ID_LOT = DI.ID_LOT WHERE DI.ID_BILL_GLOBAL = D.ID_BILL_GLOBAL),
		SUP_VAT = (SELECT SUM(L.PVAT_SUP * DI.QUANTITY) FROM BILL_ITEM DI INNER JOIN LOT L ON L.ID_LOT = DI.ID_LOT WHERE DI.ID_BILL_GLOBAL = D.ID_BILL_GLOBAL),
		VAT_10 = (SELECT SUM(LM.SVAT_ACC) FROM LOT_MOVEMENT LM INNER JOIN LOT L ON L.ID_LOT_GLOBAL = LM.ID_LOT_GLOBAL WHERE L.VAT_SAL = 10 AND LM.ID_DOCUMENT = D.ID_BILL_GLOBAL),
		VAT_18 = (SELECT SUM(LM.SVAT_ACC) FROM LOT_MOVEMENT LM INNER JOIN LOT L ON L.ID_LOT_GLOBAL = LM.ID_LOT_GLOBAL WHERE L.VAT_SAL = 18 AND LM.ID_DOCUMENT = D.ID_BILL_GLOBAL)
	FROM BILL D
		INNER JOIN STORE S_FROM ON S_FROM.ID_STORE = D.ID_STORE
		INNER JOIN CONTRACTOR C_FROM ON C_FROM.ID_CONTRACTOR = S_FROM.ID_CONTRACTOR
		INNER JOIN CONTRACTOR C_TO ON C_TO.ID_CONTRACTOR = D.ID_CONTRACTOR
...
BEGIN
	...
	SELECT
		...
		SUP_SUM = D.SUM_SUP,
		SUP_VAT = D.SVAT_SUP,
		VAT_10 = (SELECT SUM(PVAT_SAL * QUANTITY) FROM INVENTORY_VED_ITEM WHERE VAT_SAL = 10 AND ID_INVENTORY_VED_GLOBAL = D.ID_INVENTORY_VED_GLOBAL),
		VAT_18 = (SELECT SUM(PVAT_SAL * QUANTITY) FROM INVENTORY_VED_ITEM WHERE VAT_SAL = 18 AND ID_INVENTORY_VED_GLOBAL = D.ID_INVENTORY_VED_GLOBAL)
	FROM INVENTORY_VED D
		INNER JOIN STORE S_FROM ON S_FROM.ID_STORE = D.ID_STORE
		INNER JOIN CONTRACTOR C_FROM ON C_FROM.ID_CONTRACTOR = S_FROM.ID_CONTRACTOR
	WHERE D.DOC_DATE BETWEEN @DATE_FROM AND @DATE_TO AND
...
BEGIN
	...
	SELECT
		...
		SUP_SUM = D.SUM_SUP,
		SUP_VAT = D.SVAT_SUP,
		VAT_10 = (SELECT SUM(PVAT_SAL * QUANTITY) FROM INVENTORY_VED_ITEM IVI INNER JOIN INVENTORY_VED IV ON IV.ID_INVENTORY_VED_GLOBAL = IVI.ID_INVENTORY_VED_GLOBAL WHERE IVI.VAT_SAL = 10 AND IV.ID_INVENTORY_SVED_GLOBAL = D.ID_INVENTORY_GLOBAL),
		VAT_18 = (SELECT SUM(PVAT_SAL * QUANTITY) FROM INVENTORY_VED_ITEM IVI INNER JOIN INVENTORY_VED IV ON IV.ID_INVENTORY_VED_GLOBAL = IVI.ID_INVENTORY_VED_GLOBAL WHERE IVI.VAT_SAL = 18 AND IV.ID_INVENTORY_SVED_GLOBAL = D.ID_INVENTORY_GLOBAL)
	FROM INVENTORY_SVED D
		INNER JOIN STORE S_FROM ON S_FROM.ID_STORE = D.ID_STORE
		INNER JOIN CONTRACTOR C_FROM ON C_FROM.ID_CONTRACTOR = S_FROM.ID_CONTRACTOR
	WHERE D.DOC_DATE BETWEEN @DATE_FROM AND @DATE_TO AND
...
BEGIN
	...
	SELECT
		...
		SUP_SUM = (SELECT SUM(SUM_SUP) FROM VAT_CORRECT_ITEM WHERE ID_VAT_CORRECT_GLOBAL = D.ID_VAT_CORRECT_GLOBAL),
		SUP_VAT = (SELECT SUM(SVAT_SUP) FROM VAT_CORRECT_ITEM WHERE ID_VAT_CORRECT_GLOBAL = D.ID_VAT_CORRECT_GLOBAL),
		VAT_10 = (SELECT SUM(SVAT_SAL) FROM VAT_CORRECT_ITEM WHERE VAT_RATE = 10 AND ID_VAT_CORRECT_GLOBAL = D.ID_VAT_CORRECT_GLOBAL),
		VAT_18 = (SELECT SUM(SVAT_SAL) FROM VAT_CORRECT_ITEM WHERE VAT_RATE = 18 AND ID_VAT_CORRECT_GLOBAL = D.ID_VAT_CORRECT_GLOBAL)
	FROM VAT_CORRECT D
		INNER JOIN STORE S_FROM ON S_FROM.ID_STORE = D.ID_STORE
		INNER JOIN CONTRACTOR C_FROM ON C_FROM.ID_CONTRACTOR = S_FROM.ID_CONTRACTOR
	WHERE D.DOC_DATE BETWEEN @DATE_FROM AND @DATE_TO AND
...
BEGIN
	...
	SELECT
		...
		SUP_SUM = D.SUM_SUPPLIER,
		SUP_VAT = D.SVAT_SUPPLIER,
		VAT_10 = (SELECT SUM(LM.SVAT_ACC) FROM LOT_MOVEMENT LM INNER JOIN LOT L ON L.ID_LOT_GLOBAL = LM.ID_LOT_GLOBAL WHERE L.VAT_SAL = 10 AND LM.ID_DOCUMENT = D.ID_IMPORT_REMAINS_GLOBAL),
		VAT_18 = (SELECT SUM(LM.SVAT_ACC) FROM LOT_MOVEMENT LM INNER JOIN LOT L ON L.ID_LOT_GLOBAL = LM.ID_LOT_GLOBAL WHERE L.VAT_SAL = 18 AND LM.ID_DOCUMENT = D.ID_IMPORT_REMAINS_GLOBAL)
	FROM IMPORT_REMAINS D
		INNER JOIN STORE S ON S.ID_STORE = D.ID_STORE
		INNER JOIN CONTRACTOR C ON C.ID_CONTRACTOR = S.ID_CONTRACTOR
	WHERE D.DOCUMENT_DATE BETWEEN @DATE_FROM AND @DATE_TO
...
BEGIN
	...
	SELECT
		...
		SUP_VAT = D.SVAT_SUPPLIER,
		BASE_DOC = NULL,
		VAT_10 = (SELECT SUM(LM.SVAT_ACC) FROM LOT_MOVEMENT LM INNER JOIN LOT L ON L.ID_LOT_GLOBAL = LM.ID_LOT_GLOBAL WHERE L.VAT_SUP = 10 AND D.ID_STORE_FROM_TRANSIT = L.ID_STORE AND LM.ID_DOCUMENT = D.ID_INTERFIRM_MOVING_GLOBAL),
		VAT_18 = (SELECT SUM(LM.SVAT_ACC) FROM LOT_MOVEMENT LM INNER JOIN LOT L ON L.ID_LOT_GLOBAL = LM.ID_LOT_GLOBAL WHERE L.VAT_SUP = 18 AND D.ID_STORE_FROM_TRANSIT = L.ID_STORE AND LM.ID_DOCUMENT = D.ID_INTERFIRM_MOVING_GLOBAL)
	FROM INTERFIRM_MOVING D
		INNER JOIN STORE S_FROM ON S_FROM.ID_STORE = D.ID_STORE_FROM_MAIN
		INNER JOIN CONTRACTOR C_FROM ON C_FROM.ID_CONTRACTOR = S_FROM.ID_CONTRACTOR
		INNER JOIN STORE S_TO ON S_TO.ID_STORE = D.ID_STORE_FROM_TRANSIT
...
BEGIN
	...
	SELECT
		...
		SUP_VAT = D.SVAT_SUPPLIER,
		BASE_DOC = NULL,
		VAT_10 = (SELECT SUM(LM.SVAT_ACC) FROM LOT_MOVEMENT LM INNER JOIN LOT L ON L.ID_LOT_GLOBAL = LM.ID_LOT_GLOBAL WHERE L.VAT_SUP = 10 AND D.ID_STORE_TO_MAIN = L.ID_STORE AND LM.ID_DOCUMENT = D.ID_INTERFIRM_MOVING_ACCEPTANCE_ACT_GLOBAL),
		VAT_18 = (SELECT SUM(LM.SVAT_ACC) FROM LOT_MOVEMENT LM INNER JOIN LOT L ON L.ID_LOT_GLOBAL = LM.ID_LOT_GLOBAL WHERE L.VAT_SUP = 18 AND D.ID_STORE_TO_MAIN = L.ID_STORE AND LM.ID_DOCUMENT = D.ID_INTERFIRM_MOVING_ACCEPTANCE_ACT_GLOBAL)
	FROM INTERFIRM_MOVING_ACCEPTANCE_ACT D
		INNER JOIN STORE S_FROM ON S_FROM.ID_STORE = D.ID_STORE_FROM_TRANSIT
		INNER JOIN CONTRACTOR C_FROM ON C_FROM.ID_CONTRACTOR = S_FROM.ID_CONTRACTOR
		INNER JOIN STORE S_TO ON S_TO.ID_STORE = D.ID_STORE_TO_MAIN
...
BEGIN
	...
	SELECT
		...
		SUP_VAT = D.SVAT_SUPPLIER,
		BASE_DOC = NULL,
		VAT_10 = (SELECT SUM(LM.SVAT_ACC) FROM LOT_MOVEMENT LM INNER JOIN LOT L ON L.ID_LOT_GLOBAL = LM.ID_LOT_GLOBAL WHERE L.VAT_SUP = 10 AND D.ID_STORE_FROM_MAIN = L.ID_STORE AND LM.ID_DOCUMENT = D.ID_INTERFIRM_MOVING_REFUSAL_ACT_GLOBAL),
		VAT_18 = (SELECT SUM(LM.SVAT_ACC) FROM LOT_MOVEMENT LM INNER JOIN LOT L ON L.ID_LOT_GLOBAL = LM.ID_LOT_GLOBAL WHERE L.VAT_SUP = 18 AND D.ID_STORE_FROM_MAIN = L.ID_STORE AND LM.ID_DOCUMENT = D.ID_INTERFIRM_MOVING_REFUSAL_ACT_GLOBAL)
	FROM INTERFIRM_MOVING_REFUSAL_ACT D
		INNER JOIN STORE S_FROM ON S_FROM.ID_STORE = D.ID_STORE_FROM_TRANSIT
		INNER JOIN CONTRACTOR C_FROM ON C_FROM.ID_CONTRACTOR = S_FROM.ID_CONTRACTOR
		INNER JOIN STORE S_TO ON S_TO.ID_STORE = D.ID_STORE_FROM_MAIN
...
/*
exec REPEX_DOCS_REGISTRY_SAL N'
<XML>
	<DATE_FROM>2009-12-01T16:36:25.046</DATE_FROM>
	<DATE_TO>2009-12-29T16:36:25.046</DATE_TO>
	<SORT_DOC>0</SORT_DOC>
...
EXEC DBO.REMOVE_REPORT_BY_TYPE_NAME 'DocRegistryTotalEx.DocsRegistryTotalParams'