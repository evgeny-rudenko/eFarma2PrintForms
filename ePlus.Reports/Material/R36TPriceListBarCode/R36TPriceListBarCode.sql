...
IF OBJECT_ID('DBO.REPEX_R36T_PRICE_LIST_BARCODE') IS NULL EXEC('CREATE PROCEDURE DBO.REPEX_R36T_PRICE_LIST_BARCODE AS RETURN')
GO
ALTER  PROCEDURE DBO.REPEX_R36T_PRICE_LIST_BARCODE
	@XMLPARAM NTEXT AS

DECLARE @HDOC INT, @SORT INT
DECLARE @TEXT NVARCHAR(4000)
DECLARE @DATE_OST DATETIME
DECLARE @SHOW_RESERVE BIT
...
SELECT OST.ID_GOODS,
	  ...
	  OST.QUANTITY_RES,
	  REG_SERT = ISNULL(REG_CERT.NAME, '-'),
	  EXTERNAL_BARCODE = ISNULL(MV_BAR_CODE.CODE,'-')
INTO #TEMP
FROM #OST OST
...
LEFT JOIN REG_CERT ON L.ID_REG_CERT_GLOBAL = REG_CERT.ID_REG_CERT_GLOBAL
LEFT JOIN MV_BAR_CODE ON L.ID_GOODS = MV_BAR_CODE.ID_GOODS
WHERE AMOUNT_OST > 0 OR (OST.QUANTITY_RES > 0 AND @SHOW_RESERVE = 1)
