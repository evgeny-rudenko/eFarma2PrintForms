...
{
	public partial class F36RPriceListParams : ExternalReportForm, IExternalReportFormMethods
	{
		public F36RPriceListParams()
		{
			InitializeComponent();
		}
      ...
      get
      {
		...
		public void Print(string[] reportFiles)
		{
            ...
            if (cbForm.SelectedIndex == 0)
                rep.ReportPath = Path.Combine(Path.GetDirectoryName(reportFiles[0]), "R36TPriceListBarCode1.rdlc");
			if (cbForm.SelectedIndex == 1)
                rep.ReportPath = Path.Combine(Path.GetDirectoryName(reportFiles[0]), "R36TPriceListBarCode2.rdlc");

            rep.LoadData("REPEX_R36T_PRICE_LIST_BARCODE", doc.InnerXml);
            rep.BindDataSource("R36TPriceListDS_Table", 0);
            rep.BindDataSource("R36TPriceListDS_Table1", 1);
            ...
            rep.AddParameter("VER", this.GetType().Assembly.GetName().Name); 
		...
		{
			get { return "Прайс-лист. Рег. серт. внешний штрих код"; }
		}
