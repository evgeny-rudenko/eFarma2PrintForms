...
SELECT
    ...
    DAY_DOC=day(I.DATE),
	MONTH_DOC=case 
	when MONTH(I.DATE)='1' then 'января'
	when MONTH(I.DATE)='2' then 'февраля'
	when MONTH(I.DATE)='3' then 'марта'
	when MONTH(I.DATE)='4' then 'апреля'
	when MONTH(I.DATE)='5' then 'мая'
	when MONTH(I.DATE)='6' then 'июня'
	when MONTH(I.DATE)='7' then 'июля'
	when MONTH(I.DATE)='8' then 'августа'
	when MONTH(I.DATE)='9' then 'сентября'
	when MONTH(I.DATE)='10' then 'октября'
	when MONTH(I.DATE)='11' then 'ноября'
	when MONTH(I.DATE)='12' then 'декабря'
	end,  
	YEAR_DOC=YEAR(I.date),
	--доверенность
	DOV_NUM=I.AUTH_NUM,
	DOV_DAY=day(I.AUTH_DATE),
	DOV_MONTH=case
	when MONTH(I.auth_date)='1' then 'января'
	when MONTH(I.auth_date)='2' then 'февраля'
	when MONTH(I.auth_date)='3' then 'марта'
	when MONTH(I.auth_date)='4' then 'апреля'
	when MONTH(I.auth_date)='5' then 'мая'
	when MONTH(I.auth_date)='6' then 'июня'
	when MONTH(I.auth_date)='7' then 'июля'
	when MONTH(I.auth_date)='8' then 'августа'
	when MONTH(I.auth_date)='9' then 'сентября'
	when MONTH(I.auth_date)='10' then 'октября'
	when MONTH(I.auth_date)='11' then 'ноября'
	when MONTH(I.auth_date)='12' then 'декабря'
	end,  
	DOV_YEAR=YEAR(I.auth_date),
	DOV_MAN=I.AUTH_VALID_PERIOD
/*
	CONTRACTOR_PRICE_PER_UNIT = CASE WHEN I.IS_SUPPLIER = 0 
...
select 
	FIO_DIR=c.DIRECTOR_FIO,
	FIO_BUH=c.BUH_FIO,
	FIO_MOL=e.LAST_NAME+' '+left(e.FIRST_NAME,1)+'. '+left(e.MIDDLE_NAME,1)+'.'
...
--exec REPEX_INVOICE_OUT_TORG12 @XMLPARAM = N'<XML><ID_INVOICE_OUT_GLOBAL>584B390A-84DD-400D-80CF-F074CAA59D16</ID_INVOICE_OUT_GLOBAL></XML>'