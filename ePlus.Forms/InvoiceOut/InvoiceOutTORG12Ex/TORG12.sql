...
IF (OBJECT_ID('DBO.FN_FORMAT_PERCENT') IS NULL)	EXEC ('CREATE FUNCTION DBO.FN_FORMAT_PERCENT() RETURNS VARCHAR(10) AS BEGIN RETURN CONVERT(VARCHAR(10), NULL) END')
GO
ALTER FUNCTION DBO.FN_FORMAT_PERCENT(
    @Value MONEY
)
RETURNS VARCHAR(10)
AS
BEGIN
	...
	begin
		return cast(cast(round(@Value, 0) as int) as varchar(9)) + '%'
	end
	return ''
END
GO
...
SELECT
    ...
    SUM_CONTRACTOR_PRICE = CASE WHEN I.IS_SUPPLIER = 0 
        ...
        END, --12
    VAT_RATE = CASE
		WHEN I.IS_SUPPLIER = 0 THEN CASE WHEN II.PSUM_SAL = 0 THEN CAST('Без НДС' AS VARCHAR(10)) 
            ELSE DBO.FN_FORMAT_PERCENT(L.VAT_SAL) END
        ELSE CASE WHEN C.USE_VAT = 1 THEN DBO.FN_FORMAT_PERCENT(CASE WHEN C_TO.USE_VAT = 1 THEN L.VAT_SUP ELSE T.TAX_RATE END)
            ELSE '' END
        END, --13
...
SELECT TOP 1
	...
	SUPPLIER_DESCRIPT =
		COALESCE(SUP.LEGAL_PERS_SHORT, SUP.LEGAL_PERS, '') 
	...
	STORE_NAME = COALESCE(SUP.NAME, ''),
	CUSTOMER_DESCRIPT = COALESCE(CONT.FULL_NAME,'')
	...
	SUPPLIER_NAME =  ISNULL(SUP.LEGAL_PERS_SHORT, '')
		--+ COALESCE(' ИНН: ' + SUP.INN, '') +--????????????????
		+ COALESCE(' Адрес: ' + SUP.UR_ADDRESS, '')
		--+ COALESCE(' Телефон: ' + SUP.PHONE, '')
		--+ COALESCE(' КПП: ' + SUP.KPP, '')
		...
		+ COALESCE(' БИК: ' + SUP.BIK, '')
		+ COALESCE(' к/с: ' + SUP.CORR_ACCOUNT, ''),
	PAYER_NAME = COALESCE(PAY.FULL_NAME,'')
	...
	INCOMING_NUMBER = ' Договор поставки',
