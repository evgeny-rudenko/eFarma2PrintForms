...
declare @sal bit
select @sal = case when value = 'SAL' then 1 else 0 end from SYS_OPTION where code = 'INVENTORY_CALC_PRICE'
--select @sal
...
SELECT 
	...
	PRICE_SUPPLIER = case when @sal = 0 then M.PRICE_SUPPLIER else M.PRICE_SALE end,
	PRICESUMMA = case when @sal = 0 then M.PRICE_SUPPLIER else M.PRICE_SALE end * M.QUANTITY
...
SELECT
	MNEMOCODE = substring(M.MNEMOCODE, charindex('/', M.MNEMOCODE, 0) + 1, 300),
	M.DATE,
	[NAME] = CASE WHEN ISNULL(C.FULL_NAME, '') = '' THEN C.NAME ELSE C.FULL_NAME END,
	C.OKPO,
...
--exec REPEX_TORG13 N'<XML><ID_MOVEMENT>264</ID_MOVEMENT></XML>'
