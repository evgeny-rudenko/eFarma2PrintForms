...
namespace FCBActReturnToContractor
{
	public class ActReturnToContractorEx : AbstractDocumentReport, IExternalDocumentPrintForm
	{
		...
		{
            Stream s = this.GetType().Assembly.GetManifestResourceStream("FCBActReturnToContractor.ActReturnToContractor.sql");
		...
		void ExtractReport()
		{
            ...
            Stream s = this.GetType().Assembly.GetManifestResourceStream("FCBActReturnToContractor.ActReturnToContractor.rdlc");
		...
		public override IReportForm GetReportForm(DataRowItem dataRowItem)
		{
			...
			decimal dPm_SumSupp = 0, dPm_SumPvatSupp = 0,
				dPm_SumSal = 0, dPm_SumPvatSal = 0,
				dPm_SumDiscount = 0, dPm_SumTaxSale = 0;
			...
			for (int i = 0; i < dataSet.Tables[2].Rows.Count; i++)
			{
                ...
                dPm_SumSupp +=AsDecimal(dataSet.Tables[2].Rows[i]["ACI_PRICESUPP"]) * AsDecimal(dataSet.Tables[2].Rows[i]["ACI_QUANTITY"]);
                dPm_SumSal +=AsDecimal(dataSet.Tables[2].Rows[i]["ACI_PRICESAL"]) * AsDecimal(dataSet.Tables[2].Rows[i]["ACI_QUANTITY"]);
			}
