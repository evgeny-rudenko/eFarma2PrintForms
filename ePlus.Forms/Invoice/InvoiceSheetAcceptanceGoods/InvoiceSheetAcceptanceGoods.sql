...
IF OBJECT_ID('[REPEX_INVOICE_SHEET_ACCEPTANCE_GOODS]') IS NULL EXEC('CREATE PROCEDURE [REPEX_INVOICE_SHEET_ACCEPTANCE_GOODS] AS RETURN')
GO
ALTER PROCEDURE REPEX_INVOICE_SHEET_ACCEPTANCE_GOODS
	(@XMLPARAM NTEXT) AS
...
SELECT
    ...
    --яПНЙ ЦНДМНЯРХ ЯЙНППЕЙРХПНБЮММШИ
    BEST_BEFORE_CORRECT = NULL,
    --яЕПХЪ
    SERIES_NUMBER = ISNULL(SER.SERIES_NUMBER,''),
    --╧ ЙНПНАЮ
    BOX = ITEM.BOX,
    --БМСРПЕММХИ ЬРПХУ ЙНД
    INTERNAL_BARCODE = L.INTERNAL_BARCODE,
    --ЙНКХВЕЯРБН ОН ДНЙСЛЕМРС 
    QUANTITY = CAST(ROUND(ITEM.QUANTITY, 0) AS VARCHAR),
	--ТЮЙРХВЕЯЙНЕ ЙНКХВЕЯРБН 
	QUANTITY_FACT = NULL,
	--лЕЯРН УПЮМЕМХЪ  ДЮММШЕ ХГ ЯОПЮБНВМХЙЮ МНЛЕМЙКЮРСПШ
	STORE_PLACE = SP.NAME
	--select NEWID()
   /* --опнхгбндхрекэ
...
--exec REPEX_INVOICE_SHEET_ACCEPTANCE_GOODS N'<XML><ID_INVOICE>2868</ID_INVOICE></XML>'
