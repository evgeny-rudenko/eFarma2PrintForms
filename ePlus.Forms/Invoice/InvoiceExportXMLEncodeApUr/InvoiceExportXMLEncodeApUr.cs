...
namespace FCChInvoiceExportXMLEncodeApUr
{
    public class InvoiceOutExportXML : AbstractDocumentPrintForm, IExternalDocumentPrintForm 
    {
        ...
        {
            using (InvoiceExportXMLEncodeApUrForm ParamForm = new InvoiceExportXMLEncodeApUrForm())
            {
                ...
                if (ParamForm.ShowDialog() == DialogResult.OK)
                {
                    ...
                    ED.Export("USP_EXPORT_INVOICE_2_XML_AP_UR", dataRowItem.Guid);
        ...
        private void LoadSettings(int iD_CONTRACTOR_TO, InvoiceExportXMLEncodeApUrForm paramForm)
        {
        ...
        private void SaveSettings(int iD_CONTRACTOR_TO, InvoiceExportXMLEncodeApUrForm paramForm)
        {
            XmlDocument doc = new XmlDocument();
            XmlNode root;
        ...
        {
            get { return "Экспорт в формате XML-\"СПО Аптека-Урал\" (настройка кодовой страницы)"; }
        }
