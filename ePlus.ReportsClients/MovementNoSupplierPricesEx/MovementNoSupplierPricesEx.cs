...
namespace MovementNoSupplierPricesEx
{
	public class MovementNoSupplierPricesEx : AbstractDocumentReport, IExternalDocumentPrintForm
	{
		...
		{
			Stream s = this.GetType().Assembly.GetManifestResourceStream("MovementNoSupplierPricesEx.Movement.sql");
		...
		void ExtractReport()
		{
			...
			Stream s = this.GetType().Assembly.GetManifestResourceStream("MovementNoSupplierPricesEx.MovementNoSupplierPrices.rdlc");
			using (StreamReader sr = new StreamReader(s))
			{
				using (StreamWriter sw = new StreamWriter(Path.Combine(cachePath, "MovementNoSupplierPrices.rdlc")))
				{
		...
		public override IReportForm GetReportForm(DataRowItem dataRowItem)
		{
			...
			rep.ReportPath = Path.Combine(Path.Combine(folderPath, CACHE_FOLDER), "MovementNoSupplierPrices.rdlc");
		...
		{
			get { return "Перемещение товара: Перемещение (без цен поставщика)"; }
		}
	}
}
