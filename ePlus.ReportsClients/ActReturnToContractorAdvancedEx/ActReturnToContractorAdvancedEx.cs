...
namespace FCSActReturnToContractorAdvanced
{
	public class ActReturnToContractorAdvancedEx : AbstractDocumentReport, IExternalDocumentPrintForm
	{
		...
		{
            Stream s = this.GetType().Assembly.GetManifestResourceStream("FCSActReturnToContractorAdvanced.ActReturnToContractorAdvanced.sql");
		...
		void ExtractReport()
		{
            ...
            Stream s = this.GetType().Assembly.GetManifestResourceStream("FCSActReturnToContractorAdvanced.ActReturnToContractorAdvanced.rdlc");
			using (StreamReader sr = new StreamReader(s))
			{
				using (StreamWriter sw = new StreamWriter(Path.Combine(cachePath, "ActReturnToContractorAdvanced.rdlc")))
				{
		...
		public override IReportForm GetReportForm(DataRowItem dataRowItem)
		{
			...
			rep.ReportPath = Path.Combine(Path.Combine(folderPath, CACHE_FOLDER), "ActReturnToContractorAdvanced.rdlc");
			...
			ReportParameter[] parameters = new ReportParameter[9] {
				...
				AsString(dataSet.Tables[1].Rows[0]["AC_NUMBER"]) + " от " + 
				AsString(dataSet.Tables[1].Rows[0]["AC_DATE"])),
				...
				new ReportParameter("Pm_SumSalRusWords", AsRusCurrency((double) dPm_SumSupp)),
		...
		{
			get { return "Акты возврата поставщику: Возврат поставщику с розн. ценами"; }
		}
	}
}
