...
namespace InvoiceWOAdprice
{
	public class InvoiceWOAdpriceEx : AbstractDocumentReport, IExternalDocumentPrintForm
	{
		...
		{
			Stream s = this.GetType().Assembly.GetManifestResourceStream("InvoiceWOAdpriceEx.Invoice.sql");
		...
		void ExtractReport()
		{
			...
			Stream s = this.GetType().Assembly.GetManifestResourceStream("InvoiceWOAdpriceEx.InvoiceWOAdprice.rdlc");
			using (StreamReader sr = new StreamReader(s))
			{
				using (StreamWriter sw = new StreamWriter(Path.Combine(cachePath, "InvoiceWOAdprice.rdlc")))
				{
		...
		public override IReportForm GetReportForm(DataRowItem dataRowItem)
		{
			...
			string file = Path.Combine(cachePath, "InvoiceWOAdprice.rdlc");
		...
		{
			get { return "Приходные документы: Накладная (без % наценки)"; }
		}
	}
}
