IF (OBJECT_ID('INVOICE_OUT_PROTOCOL_KONS_ROZ') IS NULL) EXEC ('CREATE PROCEDURE INVOICE_OUT_PROTOCOL_KONS_ROZ AS RETURN')
GO
ALTER PROCEDURE INVOICE_OUT_PROTOCOL_KONS_ROZ(
    @XMLPARAM NTEXT
)
AS
    DECLARE @HDOC INT
...
-- 		TORG_N = CASE WHEN ISNULL(II.SUPPLIER_ADPRICE,0) = 0 THEN 
-- 						CASE WHEN ISNULL(L.PRICE_PROD,0) != 0 THEN 
-- 							((IOI.PRICE_SAL - IOI.PVAT_SAL) - L.PRICE_SUP * (((L.PRICE_SUP - L.PVAT_SUP)/L.PRICE_PROD - 1)* 100) / 100 - (L.PRICE_SUP - L.PVAT_SUP)) * 100 / L.PRICE_PROD 
-- 								ELSE NULL END 
-- 							ELSE ((IOI.PRICE_SAL - IOI.PVAT_SAL) - L.PRICE_SUP * II.SUPPLIER_ADPRICE / 100 - (L.PRICE_SUP - L.PVAT_SUP)) * 100 / L.PRICE_PROD END,--(L.PRICE_SAL - L.VAT_SUP/100*L.PRICE_SUP)*100/L.PRICE_SUP-100, --ТОРГОВАЯ НАДБАВКА

		TORG_N = CASE WHEN L.PRICE_SUP !=0 THEN IOI.PRICE_SAL*100/L.PRICE_SUP-100 ELSE 0 END, --L.PRICE_SAL*100/L.PRICE_SUP-100, --ТОРГОВАЯ НАДБАВКА
        PRICE_SAL = IOI.SUM_SAL / IOI.QUANTITY,--L.PRICE_SAL * CONVERT(MONEY, SR.DENOMINATOR) / SR.NUMERATOR,   --ЦЕНА РОЗН БЕЗ НДС
...
--exec INVOICE_OUT_PROTOCOL_KONS @xmlParam = N'<XML><ID_GLOBAL>34e42a4a-420c-48fe-90f8-7823a1aef4a6</ID_GLOBAL></XML>'
--exec INVOICE_OUT_PROTOCOL_KONS_ROZ @xmlParam = N'<XML><ID_GLOBAL>b8d19109-5a88-4756-88e1-6ef0e1164683</ID_GLOBAL></XML>'
--exec INVOICE_OUT_PROTOCOL_KONS_ROZ @xmlParam = N'<XML><ID_GLOBAL>32954c83-29f3-42ed-81f2-031e5d2f9f83</ID_GLOBAL></XML>'
